-- Verifies no rows were added or deleted — transformation only updates amount, so row count should be identical.
...
FROM (SELECT COUNT(*) AS row_count FROM analytics.orders) prod,
     (SELECT COUNT(*) AS row_count FROM analytics.sbx_orders_20260402_1641) sbx
